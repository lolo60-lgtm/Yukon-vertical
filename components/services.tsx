...
export function Services() {
  return (
    <section id="services" className="py-20 md:py-28">
      ...
      {/* Featured courses — equal height fix */}
      <div className="mx-auto max-w-7xl px-4 py-16">
        {/* items-stretch ensures both cards stretch to same height */}
        <div className="grid gap-8 md:grid-cols-2 items-stretch">
          {services
            .filter((s) => s.featured)
            .map((service, i) => (
              <AnimatedCard key={service.title} delay={i * 150}>
                {/* h-full makes the inner card fill the grid cell */}
                <div className="group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-shadow duration-300 hover:shadow-xl">
                  ...
                  {/* flex-1 makes this content area grow to fill remaining space */}
